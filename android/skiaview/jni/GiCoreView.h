//! \file GiCoreView.h
//! \brief 定义支持Android平台的图形视图类 GiCoreView
...
{
public:
    GiCoreView(GiCanvasBase* canvas);
    virtual ~GiCoreView();

    //! 检查name对应的图片边框是否存在
    bool findShapeByImageId(const char* name);

    //! 添加图片边框
    bool addImageShapes(const char* name, int width, int height);
    ...
    //! 得到重绘次数，可用于判断是否需要调用 getDynamicShapes
    int getRedrawCount();

    //! 得到正式图形内容改变次数，可用于判断是否需要调用 record
    int getChangeCount();
    ...
    //! 得到当前图形的各种度量尺寸
    int getDimensions(mgvector<float>& vars, mgvector<char>& types);

private:
    bool handleSelectionTwoFingers(const Point2d& pt1, const Point2d& pt2, int gestureState);
    bool dynZoom(const Point2d& pt1, const Point2d& pt2, int gestureState);
    bool switchZoom(const Point2d& pt);

private:
    MgViewProxyAndr*    _view;
    int                 _zoomMask;
