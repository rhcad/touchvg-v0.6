...
{
    return !!MgImageShape::findShapeByImageID(_view->_shapes, name);
}
...
bool GiCoreView::onDynDraw(GiCanvasBase& canvas)
{
    /*if (_shapesDynamic) {
        ...
        return locker.locked() && locker.shapes->draw(canvas.gs()) > 0;
    }
    else */{
        giInterlockedIncrement(&_redrawCount);
...
int GiCoreView::getDimensions(mgvector<float>& vars, mgvector<char>& types)
{
    int count = mgMin(vars.count(), types.count());
    return mgGetDimensions(_view, vars.address(), types.address(), count);
}
