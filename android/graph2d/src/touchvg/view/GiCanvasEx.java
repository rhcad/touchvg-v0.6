...
//! \brief 定义Android画布适配器类 GiCanvasEx
...
public class GiCanvasEx extends GiCanvasBase{
    ...
    public boolean drawHandle(float x, float y, int index) {
        Bitmap bmp = mView.getHandleBitmap(index);
        if (bmp != null) {
    ...
    public boolean isContextActionsVisible() {
        return mView.isContextActionsVisible();
    }

    @Override
    public boolean showContextActions(Ints actions, float x, float y, float w, float h) {
        return mView.showContextActions(actions, x, y, w, h);
    }
    ...
    public boolean drawImage(String name, float xc, float yc, float w, float h, float angle) {
        Bitmap bmp = mView.getImageBitmap(name);
    ...
    public void rawTextCenter(String text, float x, float y, float h) {
        mPen.setTextSize(72.f * h / getScreenDpi());
        mCanvas.drawText(text, x, y, mPen);
    }
}
