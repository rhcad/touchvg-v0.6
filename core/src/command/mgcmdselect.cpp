...
bool MgCmdSelect::click(const MgMotion* sender)
{
    ...
    if (!sender->pressDrag && sender->view->useFinger()
        && changed && (m_handleMode || m_handleIndex == 0)) {
...
bool MgCmdSelect::deleteSelection(MgView* view)
{
    ...
    if (shape && view->shapeWillDeleted(shape)) {
        MgShapesLock locker(view->doc(), MgShapesLock::Remove);
...
bool MgCmdSelect::groupSelection(MgView* view)
{
    MgShape* shape = m_selIds.empty() ? NULL : view->shapes()->findShape(m_selIds.front());
    int count = 0;
    
    if (shape && m_selIds.size() > 1 && view->shapeWillDeleted(shape)) {
        MgShapesLock locker(view->doc(), MgShapesLock::Remove);
        
        applyCloneShapes(view, false);

        MgShape* newgroup = view->shapes()->addShapeByType(MgGroup::Type());
...
bool MgCmdSelect::ungroupSelection(MgView* view)
{
    MgShape* shape = m_selIds.empty() ? NULL : view->shapes()->findShape(m_selIds.front());
    int count = 0;
...
bool MgCmdSelect::switchClosed(MgView* view)
{
    ...
    {
        MgShapesLock locker(view->doc(), MgShapesLock::Edit);
        MgBaseLines *lines = (MgBaseLines *)shape->shape();
