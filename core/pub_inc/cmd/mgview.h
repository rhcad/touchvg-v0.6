...
public:
    ...
    virtual void registerCommand(const char* name, MgCommand* (*creator)()) = 0; //!< 注册命令
#endif
};
...
public:
    ...
    void cancel() const { view->cancel(this); }         //!< 取消当前命令
...
*/
template <class YourCmd> inline void mgRegisterCommand(MgView* view) {
    view->registerCommand(YourCmd::Name(), YourCmd::Create);
}
#endif
